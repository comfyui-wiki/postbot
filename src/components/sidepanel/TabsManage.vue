...
<template>
  <div class="app">
    ...
    <EditorArea
      ...
      :videoUrl="localPublish.videoUrl"
      :videoFile="localPublish.videoFile"
      :videoDuration="localPublish.videoDuration"
      :videoSize="localPublish.videoSize"
      :getPlatformColor="platformColor"
      ...
      @add-video="() => localVideoInputRef?.click()"
    ...
    <input ref="localVideoInputRef" type="file" accept="video/*" style="display:none" @change="onLocalVideoSelected" />
  </div>
</template>
...
const onLocalVideoSelected = async (e: Event) => {
  const input = e.target as HTMLInputElement;
  const file = input.files?.[0];
  if (!file || !file.type.startsWith('video/')) {
    console.warn('[PostBot] 请选择有效的视频文件');
    return;
  }
  ...
  video.onloadedmetadata = () => {
    localPublish.value.videoDuration = video.duration;
    URL.revokeObjectURL(video.src);
  };
  video.src = videoUrl;

  // 保存视频文件信息
  localPublish.value.videoFile = file;
  localPublish.value.videoUrl = videoUrl;
  localPublish.value.videoSize = file.size;

  console.log('[PostBot] 视频已选择:', { name: file.name, size: file.size, type: file.type });

  input.value = '';
};

const removeVideo = () => {
  if (localPublish.value.videoUrl) {
    URL.revokeObjectURL(localPublish.value.videoUrl);
  }
  localPublish.value.videoFile = null;
  localPublish.value.videoUrl = '';
  localPublish.value.videoDuration = 0;
  localPublish.value.videoSize = 0;
  console.log('[PostBot] 视频已移除');
};
...
const triggerPublishNow = async () => {
  ...
  // Build video data for video publishing
  // Store the File object in background state, send only metadata
  let videoData = undefined;
  if (mediaType === 'video' && localPublish.value.videoFile) {
    videoData = {
      name: localPublish.value.videoFile.name,
      type: localPublish.value.videoFile.type,
      size: localPublish.value.videoSize,
      duration: localPublish.value.videoDuration
    };
    console.log('[PostBot] 视频元数据已准备:', videoData);

    // Store the File object in background script before publishing
    try {
      await new Promise<void>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('存储视频文件超时'));
        }, 5000);

        chrome.runtime.sendMessage({
          type: 'request',
          action: 'STORE_PENDING_VIDEO_FILE',
          data: {
            videoFile: localPublish.value.videoFile
          }
        }, (response) => {
          clearTimeout(timeout);
          if (chrome.runtime.lastError) {
            ...
            reject(new Error('视频文件存储失败'));
          }
        });
      });
      console.log('[PostBot] 视频文件已存储到背景脚本');
    } catch (err) {
      console.error('[PostBot] 视频文件存储失败:', err);
      alert('视频文件存储失败，请重试');
      return;
    }
  }
  ...
  chrome.runtime.sendMessage({
    ...
    data: {
      ...
      cover,
      videoData, // Video metadata only (name, type, size, duration)
      isAutoPublish: false
    }
  });
